...
Dispersion hesaba katılmadı. Multi-objective ODE + GDE3 Selection
...
class MOODE():
    ...
    def init_P0_population(self, method = 'random'):
        ...
        candidates = self.sampling(method=self.sampling_method)
    ...
    """
    def create_copy_model_from_dict(self, same_sol):
        copy_model = Model(same_sol["chromosome"], same_sol['config'], same_sol['nbr_cell'], same_sol['nbr_filters'])
        copy_model.objectives = same_sol["objectives"]
        copy_model.fitness_type = same_sol["fitness_type"]
        copy_model.hash = copy_model.get_hash()
        copy_model.encoding = copy_model.encode(predictor_encoding='trunc_path', cutoff=self.cutoff)
        self.solNo += 1
        copy_model.solNo = self.solNo
        ...
        while self.totalTrainedModel <= self.MAX_SOL:
            if G < N:
                ...
                while len(P_G) < self.pop_size:
                    ...
                    C = self.generate_candidate(X_i)
                    predicted_obj_1 = self.predictor.predict([C.encoding])[0]
                    C.fitness_type = "ESTIMATED"
                    C.objectives = [predicted_obj_1, self.get_obj_2(C, is_candidate=True)]

                    X_i = self.dict_to_model(X_i) # target - model
                    selected_sols = self.gde3_selection(self.get_model_dict(X_i, X_i.hash), self.get_model_dict(C, C.hash))
                    for sol in selected_sols:
                        P_G.append(sol)
            ...
            for p in self.population:
                if p['isFeasible'] and p['objectives'][1] == 1e4:
                    print("ERRORRR:", p['solNo'])
