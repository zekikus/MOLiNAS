...
if __name__ == "__main__":
    ...
    for seed in [10, 15, 1234]:
        ...
        model = smp.Unet(BACKBONE, encoder_weights=None, classes=n_classes, activation=activation)
        model.load_state_dict(torch.load(f"{path}/model_{BACKBONE}_seed_{seed}.pt", map_location='cuda:0'))
        model.to(device)
    ...
    result_df.to_excel(f"model_{BACKBONE}_results.xlsx")
